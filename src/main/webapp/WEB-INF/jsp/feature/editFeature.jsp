...
<div class="container">
	...
	<form method="post" action="/featureChange?userId=${loggedUser.id}&featureId=${param.featureId}&orderId=${orderId}">
		...
		<textarea rows="2" class="text-dark" name="name" cols="100">${feature.name}</textarea>
		<br /> <label>Feature Description:</label><br />
		<textarea rows="4" class="text-dark" name="description">${feature.description}</textarea>
		<br>
		<input type="checkbox" name="noStandard" value="true" size="100" <c:if test="${feature.noStandard=='true'}">
		checked
		</c:if>
		>No standard
		<br />
		...
		<input type="text" class="text-dark" name="mIndex" size="100" value="${feature.mIndex}"><br /> <br>
